...
class exfood_SC_Builder {
	...
	function save_shortcode($post_id){
		...
		if(isset($_POST['sc_type'])){
			...
			$count = intval(isset($_POST['count']) && $_POST['count'] !=''? $_POST['count'] : '9');
			if($count < 0){
				$count = 9;
			}
			$posts_per_page = intval(isset($_POST['posts_per_page'])  && $_POST['posts_per_page'] !=''? $_POST['posts_per_page'] : '3');
			if($posts_per_page < 0){
				$posts_per_page = 3;
			}
			$slidesshow = intval(isset($_POST['slidesshow']) && $_POST['slidesshow'] !=''? $_POST['slidesshow'] : '3');
			if($slidesshow < 0){
				$slidesshow = 3;
			}
			...
			$number_excerpt = intval(isset($_POST['number_excerpt']) && $_POST['number_excerpt'] !='' ? $_POST['number_excerpt'] : '10');
			if($number_excerpt < 0){
				$number_excerpt = 10;
			}
			...
			$slidesscroll = sanitize_text_field(isset($_POST['slidesscroll']) ? $_POST['slidesscroll'] : '');
			if ($slidesscroll !='') {
			...
			if(sanitize_text_field($_POST['sc_type']) == 'grid'){
				...
				$sc = '[ex_food_grid style="'.esc_attr($style).'" column="'.esc_attr($column).'" count="'.esc_attr($count).'" posts_per_page="'.esc_attr($posts_per_page).'" ids="'.esc_attr($ids).'" cat="'.esc_attr($cat).'" order="'.esc_attr($order).'" orderby="'.esc_attr($orderby).'" meta_key="'.esc_attr($meta_key).'" meta_value="'.esc_attr($meta_value).'" number_excerpt="'.esc_attr($number_excerpt).'" page_navi="'.esc_attr($page_navi).'"]';
				
			}elseif(sanitize_text_field($_POST['sc_type']) == 'list'){
				$sc = '[ex_food_list style="1" count="'.esc_attr($count).'" posts_per_page="'.esc_attr($posts_per_page).'" ids="'.esc_attr($ids).'" cat="'.esc_attr($cat).'" order="'.esc_attr($order).'" orderby="'.esc_attr($orderby).'" meta_key="'.esc_attr($meta_key).'" meta_value="'.esc_attr($meta_value).'" number_excerpt="'.esc_attr($number_excerpt).'" page_navi="'.esc_attr($page_navi).'"]';
				
			}elseif(sanitize_text_field($_POST['sc_type']) == 'table'){
	...
	function register_metabox() {
		...
		$layout->add_field( array(
			...
			'options'          => array(
				...
				'table'   => esc_html__( 'Table(Only Pro Version)', 'wp-food' ),
		...
		$sc_option->add_field( array(
			'name'       => esc_html__( 'Menu filter (Only Pro Version)', 'wp-food' ),
		...
		$sc_option->add_field( array(
			'name'       => esc_html__( 'Order Menu Filter (Only Pro Version)', 'wp-food' ),
			'desc'       => esc_html__( 'Order Menu Filter with custom order', 'wp-food' ),
			'id'         => 'order_cat',
			'type'             => 'select',
			'classes'             => 'column-2',
			'show_option_none' => false,
		...
		$sc_option->add_field( array(
			'name'       => esc_html__( 'Items On Sale (Only Pro Version)', 'wp-food' ),
			'desc'       => esc_html__( 'Select items on sale', 'wp-food' ),
			'id'         => 'on_sale',
			'type'             => 'select',
			'classes'             => 'column-2',
			'show_option_none' => false,
		...
		$sc_option->add_field( array(
			'name'       => esc_html__( 'Menu filter Position (Only Pro Version)', 'wp-food' ),
		...
		$sc_option->add_field( array(
			'name'       => esc_html__( 'Show Side cart (Only Pro Version)', 'wp-food' ),
		...
		$sc_option->add_field( array(
			'name'       => esc_html__( 'Live Sort (Only Pro Version)', 'wp-food' ),
		...
		$sc_option->add_field( array(
			'name'       => esc_html__( 'Loading effect (Only Pro Version)', 'wp-food' ),
