...
function exfood_admin_scripts(){
	$nonce = wp_create_nonce( 'food_nounce_ad' );
	$js_params = array( 'ajaxurl' => admin_url( 'admin-ajax.php' ), 'food_admin' => $nonce );
...
function exfood_change_sort(){
	$food_admin = sanitize_text_field($_POST['food_admin']);
	if ( ! wp_verify_nonce( $food_admin, 'food_nounce_ad' ) ) {
...
function exfood_table_order_item_html(){
	$post_id = intval(isset($_POST['post_id']) && $_POST['post_id']!='' ? $_POST['post_id'] : get_the_ID()) ;
...
function adm_exfood_remove_cart_item(){
	...
	$key = sanitize_text_field($_POST['it_remove']);
	$post_id = intval($_POST['post_id']);
	if(is_numeric($key)){
		$userfood = get_post_meta( $post_id, 'exorder_food', true);
		unset($userfood[$key]);
...
function adm_exfood_update_cart_item(){
	...
	$key = sanitize_text_field($_POST['it_update']);
...
function adm_exfood_add_order_meta(){
	...
	$key = sanitize_text_field($_POST['it_update']);
	$post_id = intval($_POST['post_id']);
	$metas = ex_sanitizing_array($_POST['metas']);
	if(empty($metas)){
		echo 0; exit;
	}
	if(is_numeric($key)){
...
if(!function_exists('ex_sanitizing_array')){
	function ex_sanitizing_array($array) {
		if (!isset($array)) {
			$array = array();
			return $array;
		}
		...
		foreach ( $array as $key => $val ) {
			...
			$new_input[ $key ] = ( isset( $array[ $key ] ) ) ? sanitize_text_field( $val ) : '';
		}
		return $new_input;
	}
}
...
add_action('wp_ajax_exfd_change_sort_menu', 'wp_ajax_exfd_change_sort_menu' );
function wp_ajax_exfd_change_sort_menu(){
	$food_admin = sanitize_text_field($_POST['food_admin']);
	if ( ! wp_verify_nonce( $food_admin, 'food_nounce_ad' ) ) {
	    // This nonce is not valid.
	    die( 'Security check' ); 
...
function exfd_create_help(){
    include( 'exfd-help.php' );	
}