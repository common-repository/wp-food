<?php
function exfood_shortcode_list( $atts ) {
	if(phpversion()>=7){
		$atts = (array)$atts;
	}
...
add_shortcode( 'ex_food_list', 'exfood_shortcode_list' );
add_action( 'after_setup_theme', 'ex_food_reg_food_list_vc' );
function ex_food_reg_food_list_vc(){
    if(function_exists('vc_map')){
	vc_map( array(
	   "name" => esc_html__("wp-food - List", "wp-food"),
	   "base" => "ex_food_list",
	   "class" => "",
	   "icon" => "icon-grid",
	   "controls" => "full",
	   ...
	   "params" => array(
		  ...
		  array(
			...
			"value" => "",
			"description" => esc_html__("Enter Number food per page", 'wp-food'),
		  ),
		  array(
		  	"admin_label" => true,
			"type" => "textfield",
		  ...
		  array(
			 ...
			 "type" => "dropdown",
			 "class" => "",
			 "heading" => esc_html__("Menu filter Position", 'wp-food'),
			 "param_name" => "menu_pos",
			 "value" => array(
			 	esc_html__('Top', 'wp-food') => 'top',
			 	esc_html__('Left', 'wp-food') => 'left',
			 ),
			 "description" => esc_html__("Select posstion of menu filter", "wp-food"),
		  ),
		  array(
		  	"admin_label" => true,
			 "type" => "dropdown",
