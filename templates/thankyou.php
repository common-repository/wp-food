...
<div class="exfood-thankyou">
  ...
  <div class="ex-order-billing">
    ...
    <address>
      <?php
        if ($billing['_location'] == '') {
          $name = '';
        }else{
          $termad = get_term_by('slug', $billing['_location'], 'exfood_loc');
          $name = $termad->name;
        }
        echo $billing['_fname'] !='' || $billing['_lname'] !='' ? esc_html__('Name: ','wp-food').$billing['_fname'].' '.$billing['_lname'].'<br>' : '';
        echo $billing['_location'] !='' ? esc_html__('Location: ','wp-food').$name.'<br>' : '';
  ...
  <?php 
    $exfood_enable_print = exfood_get_option('exfood_enable_print');
    if ($exfood_enable_print != 'no') {
