  ...
  foreach ($userfood as $key => $value) {
    ...
    echo '<li>
      ...
      <div class="exfood-cart-details">
        <a class="exfood-title" href="'.esc_url($customlink).'">'.get_the_title($food_id).': '.exfood_price_with_currency($price_food).'</a>';
...
echo '<p class="exfood-total"><strong>'.esc_html__('Subtotal:','wp-food').'</strong>
  ...
  <input type="hidden"  name="total_price_required" value="">
  </p>';
  if(isset($checkout) && $checkout ==1){}else{
